...
public class MWater extends LinearLayout implements View.OnTouchListener {
    LayoutInflater inflater;
    View contentView;
    ImageView temp_btn;
    ImageView temp_img;
    ImageView temp_water;

    public MWater(Context context) {
        super(context);
        initView();
    }

    public MWater(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView();
    }

    public MWater(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }
    ...
    public void initView(){
        ...
        contentView = inflater.inflate(R.layout.water_water_view,null);
        temp_btn = (ImageView) contentView.findViewById(R.id.btn);
        temp_img = (ImageView) contentView.findViewById(R.id.img);
        temp_water= (ImageView) contentView.findViewById(R.id.water);
        addView(contentView);
...
//                changeLocation(0);
    ...
    private void changeLocation(float downY,float moveY){
        ...
        RelativeLayout.LayoutParams paramsImg = (RelativeLayout.LayoutParams) temp_img.getLayoutParams();
        RelativeLayout.LayoutParams paramsWater = (RelativeLayout.LayoutParams) temp_water.getLayoutParams();
        int marginTop = paramsImg.topMargin;
        int raw = 0;
        if (moveY == downY){
        ...
        paramsWater.topMargin = raw + marginTop;
        temp_btn.requestLayout();
        temp_water.requestLayout();
